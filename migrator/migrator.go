...
func (m *Migrator) GetObjectsForStep(step int) (result []*DbObjectVersion, err error) {
	...
	for _, dbObject := range entries {
		...
		for _, version := range dbObject.Versions {
			if version > correctVersion && step >= version {
...
func (m *Migrator) CreateObjectsForStep(step int) error {
	...
	objectVersions, err := m.GetObjectsForStep(step)
	if err != nil {
		return err
	}
...
func (m *Migrator) CreateObjectsFile() error {
	...
	data = append(data, []byte("-- Generated by migrator v"+Version+" at "+time.Now().String()+"\n\n")...)

	dbObjectVersions, err := m.GetObjectsForStep(version)
	if err != nil {
		return err
	}
...
func (m *Migrator) RunStep(direction Direction, drop bool, create bool) (int, error) {
	...
	if errors.Is(err, migrate.ErrNoChange) {
		...
		err = m.CreateObjectsForStep(finalVersion)
		if err != nil {
			return currentVersion, err
		}
